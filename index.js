...
class Aichat{constructor(){this.url="https://chat-gpt.org/chat",this.working=!0,this.supports_gpt_35_turbo=!0}static async createAsync(t,r,s){const e={message:Aichat.formatPrompt(r),temperature:s?.temperature||.5,presence_penalty:0,top_p:s?.top_p||1,frequency_penalty:0},o=await got.post("https://chat-gpt.org/api/text",{json:e});if(!o.ok)throw new Error(`HTTP Error: ${o.statusCode}`);const a=JSON.parse(o.body);if(!a.response)throw new Error(`Error Response: ${JSON.stringify(a)}`);return a.message}static formatPrompt(t){return JSON.stringify(t)}}
...
const text = req.query.text;
  ...
  const messages = [{
        ...
        const response = await Aichat.createAsync(model, messages);
