...
async function detectNSFW(buffer) {
  ...
  const predictions = await model.classify(new Uint8Array(buffer));

  return predictions.map(prediction => `${prediction.className}: ${prediction.probability}`);
}
...
app.post("/nsfwdetect", async(req,res)=> {
let q = req.files.file.data
	if (!q) return res.json({msg:"Input Query"})
	let data = await detectNSFW(q)
	res.json(data)
