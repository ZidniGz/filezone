...
igdl=async function(e){let t=await got("https://indown.io/").text(),a=cheerio.load(t.data),o=a("input[name=referer]").val(),i=a("input[name=locale]").val(),n=a("input[name=_token]").val(),{data:r}=await axios.post("https://indown.io/download",new URLSearchParams({link:e,referer:o,locale:i,_token:n}),{headers:{cookie:t.headers["set-cookie"].join("; ")}}),l=cheerio.load(r),s=[],c=cheerio.load(l("#result").html());return c("video").each((function(){let e=l(this);s.push({type:"video",thumbnail:e.attr("poster"),url:e.find("source").attr("src")})})),c("img").each((function(){let e=l(this);s.push({type:"image",url:e.attr("src")})})),s[0]};
