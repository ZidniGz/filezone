...
igdl = async function (link) {
  const res = await got
    .post("https://igram.online/igram/action.php", { form: { url: link } })
    .text();
  ...
  var data = {
    title: $(".snaptik-middle.center > p > span").text(),
    thumbnail: metaTags,
    url: $(".abuttons.mb-0 > a").attr("href"),
  };
  return data;
};
 const yt_dl=t=>new Promise((async(e,i)=>{try{let i=["en","id","es"][0];const n=await got.post("https://www.y2mate.com/mates/analyzeV2/ajax",{headers:{accept:"*/*","accept-encoding":"gzip, deflate, br","content-type":"application/x-www-form-urlencoded; charset=UTF-8",cookie:"_gid=GA1.2.2055666962.1683248123; _gat_gtag_UA_84863187_21=1; _ga_K8CD7CY0TZ=GS1.1.1683248122.1.1.1683249010.0.0.0; _ga=GA1.1.1570308475.1683248122",origin:"https://www.y2mate.com","user-agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/96.0.4664.110 Safari/537.36"},form:{k_query:t,k_page:"home",hl:i,q_auto:0}}).json();function o(t){const e=parseFloat(t);return(isNaN(e)?0:e)*(/GB/i.test(t)?1e6:/MB/i.test(t)?1e3:/KB/i.test(t)?1:/bytes?/i.test(t)?.001:/B/i.test(t)?.1:0)}const s=n.vid,l={},c={};let p=[];for(const t in n.links.mp4){const e=n.links.mp4[t],i=e.q;if("mp4"!==e.f)continue;const a=e.size,c=o(a);p.push(i),l[i]={quality:i,fileSizeH:a,fileSize:c,download:`https://filezone-zeta.vercel.app/convert?id=${s}&kualitas=${e.k}`}}for(const t in n.links.mp3){const e=n.links.mp3[t],i=e.q;if("mp3"!==e.f)continue;const a=e.size,l=o(a);c[i]={quality:i,fileSizeH:a,fileSize:l,download:`https://filezone-zeta.vercel.app/convert?id=${s}&kualitas=${e.k}`}}e({id:s,thumbnail:`https://i.ytimg.com/vi/${s}/0.jpg`,title:n.title,duration:(a=n.t,`${Math.floor(a/60)}:${a%60}`),kualitas:p,video:l,audio:c})}catch(t){i(t)}var a}));
const convert2=async function(e,t){return(await got("https://www.y2mate.com/mates/convertV2/index",{method:"POST",headers:{accept:"*/*","accept-encoding":"gzip, deflate, br","content-type":"application/x-www-form-urlencoded; charset=UTF-8",cookie:"_gid=GA1.2.2055666962.1683248123; _ga=GA1.1.1570308475.1683248122; _ga_K8CD7CY0TZ=GS1.1.1683248122.1.1.1683248164.0.0.0; prefetchAd_3381349=true",origin:"https://www.y2mate.com","user-agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/96.0.4664.110 Safari/537.36"},form:{vid:e,k:t}}).json()).dlink};
